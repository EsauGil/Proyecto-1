...
void setup() {
  ...
  TDBX = 1200;                    // Posicion X del TD1B
  TDBY = 400;                     // Posicion Y del TD1B
...
void Estatus_escalas(){   // Funciones que ponen en pantalla el valor de las escalas de tiempo y voltaje de los canalaes activos
...
void Desempaquetado() {
  ...
  canal[0] = (aux[0] + aux[1])/130;       // Canal Analogico 1 Listo
  ...
  aux[1] = int(leer[3]);
  aux[1] = int(byte(aux[1]) & byte(63));
  canal[1] = (aux[0] + aux[1])/130;      // Canal Analogico 2 Listo
...
void Analog1() {
  for (int x = 0; x < 999; x += 1)  {
     ...
     if (x*ts < 1000)
       line(x*ts, 400 - analog1[x]*a1vd, (x+1)*ts, 400 - analog1[(x+1)]*a1vd); 
           }
}
...
void Analog2() {
  for (int x = 0; x < 999; x += 1)  {
     ...
     if (x*ts < 1000)
       line(x*ts, 400 - analog2[x]*a2vd, (x+1)*ts, 400 - analog2[(x+1)]*a2vd); 
           }
}
